/*
 * Server-only import handler retained from the Sites deployment.
 * GitHub Pages cannot execute this Cloudflare D1 route; keep it here as the
 * backend reference for a future independent API deployment.
 */
...
export async function POST(request: Request) {
  ...
  if (!env.DB) return json({ ok:false, message:'当前预览环境未绑定 D1；该接口需要独立后端。' }, 503);
