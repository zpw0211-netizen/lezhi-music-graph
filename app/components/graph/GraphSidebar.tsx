...
type View = "home" | "graph" | "assistant" | "research" | "records" | "work" | "import";
export function GraphSidebar({ view, onView, graph, books, scopeBook, filters, perspective, onPerspective, onSchema, onSources, onReset, execute, notice, assetUrl, onGraphFocus }: { view: View; onView: (view: View) => void; graph: RagGraph | null; books: FilterBook[]; scopeBook?: string; filters: GraphFilters; perspective: GraphPerspective; onPerspective: (value: GraphPerspective) => void; onSchema: () => void; onSources: () => void; onReset: () => void; execute: (input: unknown) => boolean; notice: string; assetUrl: (path: string) => string; onGraphFocus: (answer: AnswerResult) => void }) {
  ...
  return <aside className={`sidebar explorer-sidebar ${collapsed ? "is-collapsed" : ""}`} aria-label="图谱导航与控制">
    ...
    <nav className="explorer-nav">{([["home", "首页", "home"], ["graph", "图谱探索", "graph"], ["assistant", "智能问答", "message"], ["records", "作品档案", "book"], ["research", "研究分析", "research"]] as const).map(([key, label, icon]) => <button key={key} title={label} aria-label={label} className={view === key || (key === "records" && view === "work") ? "active" : ""} onClick={() => onView(key)}><WorkbenchIcon name={icon} />{!collapsed && label}</button>)}</nav>
    {!collapsed && view === "graph" && <div className="explorer-controls"><section className="explorer-perspective"><h3>图谱观察方式</h3><div>{([["comprehensive", "综合知识"], ["textbook", "教材结构"], ["music", "音乐知识"], ["progression", "学习进阶"]] as const).map(([key, label]) => <button key={key} aria-pressed={perspective === key} onClick={() => onPerspective(key)}>{label}</button>)}</div></section>
      ...
      <details className="explorer-section explorer-schema-section"><summary>知识模式 <small>Schema</small></summary>
        <button className="explorer-schema-button" onClick={onSchema}>查看实体与关系模式 <WorkbenchIcon name="arrow" /></button>
      </details>
      <SmartGraphQuery graph={graph} execute={execute} assetUrl={assetUrl} onGraphFocus={onGraphFocus} notice={notice} />
      <button className="explorer-reset" onClick={onReset}>恢复全部筛选</button>
    </div>}
    {!collapsed && <section className="explorer-dataset" aria-label="教材数据规模"><h3>教材数据集 <small>DATASET</small></h3><dl><div><dt>规范实体</dt><dd>{(graph?.entities.length ?? 0).toLocaleString("en-US")}</dd></div><div><dt>规范关系</dt><dd>{(graph?.relationships.length ?? 0).toLocaleString("en-US")}</dd></div></dl></section>}
