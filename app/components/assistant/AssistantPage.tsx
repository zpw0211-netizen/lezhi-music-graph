...
export function AssistantPage({ graph, assetUrl, initialQuestion = "", onGraphFocus }: { graph: RagGraph; assetUrl: (path: string) => string; initialQuestion?: string; onGraphFocus: (result: AnswerResult) => void }) {
  ...
  return <section className="assistant-page" aria-label="智能问答">
    ...
    <form className="assistant-composer" onSubmit={event => { event.preventDefault(); void submit(); }}><label className="assistant-composer-label" htmlFor="textbook-question">向教材知识图谱提问</label><textarea id="textbook-question" placeholder="输入作品、乐理问题，或探索教材之间的知识关联……" aria-label="向芽谱提问" value={question} onChange={event => setQuestion(event.target.value)} maxLength={1000} rows={2} onKeyDown={event => { if (event.key === "Enter" && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); void submit(); } }} /><div><label><select aria-label="回答模式" value={mode} onChange={e => setMode(e.target.value)}><option value="normal">普通问答</option><option value="analysis">深度分析</option></select></label><button type="submit" disabled={!question.trim() || busy}>{busy ? "检索中" : <>提问 <WorkbenchIcon name="arrow" /></>}</button></div><small>回答可追溯教材证据；补充解释不等于教材事实。AI 不会修改正式图谱。</small></form>
  </section>;
}
